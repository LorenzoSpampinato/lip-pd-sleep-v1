...
srun singularity exec \
    -B ${VOLUME}:/Dataset_bin,${PWD_APP}:/app,${SAVE_PATH}:/Results,${LABEL_PATH}:/Annotations \
    --pwd /app ${IMAGE_PATH} \
    bash -c "source /opt/conda/etc/profile.d/conda.sh && \
    conda activate ${ENV_NAME} && \
    python /app/${PYTHON_SCRIPT} --data_path /Dataset_bin --label_path /Annotations --save_path /Results --info_path /Results --only_class ${ONLY_CLASS} --only_patient ${ONLY_PATIENT}"