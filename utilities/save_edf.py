...
def write_mne_edf(mne_raw, fname, picks=None, tmin=0, tmax=None,
    ...
    """
    print(
        'did you know EDF export is now supported in MNE via edfio? have a look at https://mne.tools/stable/generated/mne.export.export_raw.html')
...
def export_eeg_to_edf(data_path1, output_path, sfreq=250, only_class='DYS', only_patient='PD011'):
    # Trova la cartella del paziente specifico
    if only_class and only_patient:
        sub_fold = os.path.join(data_path1, only_class, only_patient)
    else:
        print("Specificare una classe e un paziente.")
        return

    # Definisci il percorso completo per il file .mff
    mff_path = sub_fold + '.mff' if not sub_fold.endswith('.mff') else sub_fold

    # Controlla l'esistenza del percorso e rinomina se necessario
    if not sub_fold.endswith('.mff') and os.path.exists(sub_fold):
        print(f"Rinominando {sub_fold} in {mff_path}...")
        os.rename(sub_fold, mff_path)

    # Carica il file EEG come oggetto Raw di MNE
    raw = mne.io.read_raw_egi(mff_path, preload=True)

    # Genera il nome del file EDF di output
    edf_fname = os.path.join(output_path, f"{only_patient}_export.edf")

    # Esporta in EDF
    write_mne_edf(mne_raw=raw, fname=edf_fname, tmin=0, tmax=None, overwrite=True)

    print(f"File EDF esportato: {edf_fname}")
